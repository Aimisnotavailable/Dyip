...
from configs.arconfig import (
    ...
    PINCH_FRAMES_REQ,
    # Additional indices from your arconfig
    MIDDLE_TIP_IDX, RING_TIP_IDX, RING_MCP_IDX,
    PINKY_TIP_IDX, THUMB_MCP_IDX
)

# Note: the above indices are as defined in your arconfig.
# Make sure your arconfig defines these.
...
class FistDetector(BaseGestureDetector):
    def __init__(self, hand_label, hold_frames=None, threshold=0.5):
        super().__init__(hand_label, 'fist', hold_frames)
        self.threshold = threshold   # lowered from 0.6 for better sensitivity
    ...
    def _compute_raw_state(self, landmarks):
        ...
        # Normalize by hand scale (distance from wrist to middle MCP)
        middle_mcp = landmarks[MIDDLE_MCP_IDX]
        sx = middle_mcp[0] - wrist[0]
        sy = middle_mcp[1] - wrist[1]
        sz = middle_mcp[2] - wrist[2]
        ...
        rel_dist = total_dist / (5 * hand_scale)
        raw_active = rel_dist < self.threshold
        ...
        if self._debug_counter % 30 == 0:
            print(f"[FIST {self.hand}] avg_tip_mcp_dist={rel_dist:.3f} active={raw_active}")
