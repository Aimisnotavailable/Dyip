...
class AR:
    ...
    # MediaPipe-like hand connections (index pairs)
    HAND_CONNECTIONS = [
        (WRIST_IDX, THUMB_CMC_IDX), (THUMB_CMC_IDX, THUMB_MCP_IDX),
        (THUMB_MCP_IDX, THUMB_IP_IDX), (THUMB_IP_IDX, THUMB_TIP_IDX),
        (WRIST_IDX, INDEX_MCP_IDX), (INDEX_MCP_IDX, INDEX_PIP_IDX),
        (INDEX_PIP_IDX, INDEX_DIP_IDX), (INDEX_DIP_IDX, INDEX_TIP_IDX),
        (WRIST_IDX, MIDDLE_MCP_IDX), (MIDDLE_MCP_IDX, MIDDLE_PIP_IDX),
        (MIDDLE_PIP_IDX, MIDDLE_DIP_IDX), (MIDDLE_DIP_IDX, MIDDLE_TIP_IDX),
        (WRIST_IDX, RING_MCP_IDX), (RING_MCP_IDX, RING_PIP_IDX),
        (RING_PIP_IDX, RING_DIP_IDX), (RING_DIP_IDX, RING_TIP_IDX),
        (WRIST_IDX, PINKY_MCP_IDX), (PINKY_MCP_IDX, PINKY_PIP_IDX),
        (PINKY_PIP_IDX, PINKY_DIP_IDX), (PINKY_DIP_IDX, PINKY_TIP_IDX)
    ]

    def __init__(self, screen_dim=(1280, 720)):
        self.W = screen_dim[0]   # kept for compatibility, but not used for pixel conversions
        self.H = screen_dim[1]   # will be overridden by actual frame dimensions
        ...
        # New attributes for annotation
        self.annotated_frame = None   # will hold the drawn frame
        self.cam_w = None
        self.cam_h = None
    ...
    def _reconcile_handedness(self, detections):
        ...
        for label, pos_norm in tracked.items():
            if pos_norm is not None:
                tracked_px[label] = (pos_norm[0] * self.cam_w, pos_norm[1] * self.cam_h)
            else:
                tracked_px[label] = None

        MAX_JUMP = max(self.cam_w, self.cam_h) * 0.10
        assigned_blobs = set()
        assigned_labels = set()
        final_assignments = {}
        ...
        if len(unassigned_blobs) == 2 and len(assigned_labels) == 0:
            ...
            for b_idx, orig_label, wrist_px, _ in unassigned_blobs:
                ...
                if orig_label in avail_labels:
                    ...
                    label = "LEFT" if wrist_px[0] < self.cam_w / 2 else "RIGHT"
                final_assignments[b_idx] = label
                assigned_labels.add(label)
                assigned_blobs.add(b_idx)
    ...
    def _compute_flow_velocity(self, old_points, new_points, label):
        ...
        dx_norm = dx_px / self.cam_w
        dy_norm = dy_px / self.cam_h
    ...
    # ------------------------------------------------------------------
    # Drawing helpers for annotation
    # ------------------------------------------------------------------
    def _draw_hand_landmarks(self, frame, landmarks, label, is_ghost=False):
        """Draw hand landmarks on the given frame (BGR numpy array)."""
        h, w = frame.shape[:2]
        color = (0, 255, 0) if not is_ghost else (0, 255, 255)  # green for real, yellow for ghost
    ...
    def update(self, frame):
        ...
        if getattr(res, 'multi_hand_landmarks', None):
            for lm_set, handedness in zip(res.multi_hand_landmarks, res.multi_handedness):
                ...
                try:
                    ...
                    wrist_px = (float(lm_w.x) * self.cam_w, float(lm_w.y) * self.cam_h)
                except Exception:
                    wrist_px = None
            ...
            for i, (label, wrist_px, lm_set) in enumerate(detections):
                ...
                for idx in FLOW_POINT_INDICES:
                    if idx < len(landmarks_norm):
                        ...
                        flow_pts_px.append([x_norm * self.cam_w, y_norm * self.cam_h])
                    ...
                    if self.lk_tracked_points[label] is not None and self.prev_gray is not None:
                        ...
                        if len(self.lk_tracked_points[label]) >= 2:
                            ...
                            if st is not None and np.all(st == 1):
                                ...
                                new_wrist_px = p1[0, 0]
                                new_wrist_norm = (new_wrist_px[0] / self.cam_w, new_wrist_px[1] / self.cam_h, 0.0)
        ...
        # Create annotated frame (draw landmarks)
        if frame is not None:
            annotated = frame.copy()
            for label in ("LEFT", "RIGHT"):
                pts = ar_data["POSITION_DATA"].get(label, [])
                if pts:
                    is_ghost = (ar_data["FRAME_TYPE"][label] == "GHOST")
                    self._draw_hand_landmarks(annotated, pts, label, is_ghost)
    ...
    def get_annotated_frame(self):
        """Return a copy of the frame with hand landmarks drawn."""
